        ...
        <tbody>
          {days.map((day) => (
            <tr key={day} className="odd:bg-gray-100 even:bg-white text-center">
              <td className="border px-6 py-3 font-medium bg-gray-200">{day}</td>
              {courses.map((course) => (
                <td key={course} className="border px-6 py-3 whitespace-pre-line">
                  {routine[day][course] ? (
                    <div>
                      <span className="font-semibold text-blue-700">{routine[day][course].time}</span>
                      <br />
                      <span className="text-gray-600">{routine[day][course].room.number}</span>
                    </div>
                  ) : (
                    "—"
                  )}
